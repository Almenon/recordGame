...
function setUpEnemySpawning2(){
    game.time.events.loop(Phaser.Timer.SECOND*10, () => {
        SPEED = SPEED/1.15; //slowly increase difficulty by lowering (raising) speed
        enemyTimer.delay = enemyTimer.delay/1.15; //increase spawn frequency to match
    }); 
    setTimeout(()=> {maxTotalEnemyLen + .1},10000); //raise maxTotalEnemyLen after 10 sec for increased difficulty
    setTimeout(()=> {maxEnemyLen + .1},20000); //raise maxEnemyLen after 20 sec for increased difficulty

    enemyTimer = game.time.create();
    enemyTimer.loop(Phaser.Timer.SECOND, () => {spawn()});
    enemyTimer.start();
}

// spawn vars
var maxNumEnemies = 15;

var maxTotalEnemyLen = 1.8;

var minEnemySize = .1;
var maxEnemyLen = 1.8-minEnemySize;

var gapSize = .2;

// often results in large arcs w/ few gaps, ~3 enemygroups max
function spawn(){
    var currentTotalEnemyLen = 0;
    var previousEnd = -gapSize + Math.random()*2; // to avoid gap at beginning

    for(var i=0;i < maxNumEnemies; i++){
        var s = previousEnd + gapSize;

        var randEnemyLen = minEnemySize + Math.random() * maxEnemyLen;
        currentTotalEnemyLen += randEnemyLen;
        if(currentTotalEnemyLen >= maxTotalEnemyLen) break;

        var e = s+randEnemyLen;
        spawnEnemies(s,e);
        previousEnd = e;
    }
}
